...
float SegyReader::ibm_to_float(uint32_t ibm) const {
    // Конвертация IBM float в IEEE float
    if (ibm == 0) return 0.0f;
    
    uint32_t sign = (ibm >> 31) & 1;
    uint32_t exponent = (ibm >> 24) & 0x7F;
    uint32_t mantissa = ibm & 0xFFFFFF;
    ...
    // Конвертация в IEEE float
    uint32_t ieee = (sign << 31) | ((exponent + 127 - 64) << 23) | ((mantissa & 0x7FFFFF));
